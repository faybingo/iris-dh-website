...
export async function POST(request) {
  ...
  if (!apiKey) {
    return Response.json({ error: "OPENAI_API_KEY or OPENAI_KEY is not configured" }, { status: 500 });
  }
...
5. 语气专业、清晰、适合中国跨境卖家。`,
    ...
    if (!openaiResponse.ok) {
      return Response.json({ error: data.error?.message || "OpenAI request failed" }, { status: openaiResponse.status });
    }

    return Response.json({ analysis: extractOutputText(data) });
  } catch (error) {
